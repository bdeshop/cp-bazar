...
export default function AllUser() {
  ...
  const filteredCustomers = customerInfo?.filter((user) => {
    ...
    const currencyMatch =
      !filters.currency ||
      user.currency?.toLowerCase() === filters.currency.toLowerCase();
    const depositMatch =
  ...
  const renderUserCard = (user) => (
    <UserCard key={user._id}>
      <div>
        <CardLabel>Name:</CardLabel> <CardValue>{user.name || "-"}</CardValue>
      </div>
      <div>
      ...
      <div>
        ...
        <CardValue>{user.phoneNumber || "-"}</CardValue>
      </div>
      <div>
        <CardLabel>Currency:</CardLabel>{" "}
        <CardValue>{user.currency || "-"}</CardValue>
      </div>
      <div>
        <CardLabel>Deposit:</CardLabel>{" "}
        <CardValue>
          {user.deposit !== undefined ? `${user.deposit.toFixed(2)}` : "-"}
        </CardValue>
      </div>
      <div>
        <CardLabel>Withdraw:</CardLabel>{" "}
        <CardValue>
          {user.withdraw !== undefined ? `${user.withdraw.toFixed(2)}` : "-"}
        </CardValue>
      </div>
      <div>
            ...
            {customerInfo?.length > 0 ? (
              customerInfo.map((user) => (
                <tr key={user._id}>
                  ...
                  <td>
                    {user.deposit !== undefined
                      ? `${user.deposit.toFixed(2)}`
                      : "-"}
                  </td>
                  <td>
                    {user.withdraw !== undefined
