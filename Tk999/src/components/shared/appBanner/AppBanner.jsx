...
const PromoBanner = () => {
  ...
  const { language } = useContext(AuthContext); // Get language from AuthContext
  ...
  return (
    <div className="md:hidden bg-[#003a3a] text-white flex items-center justify-between p-2 rounded shadow-md relative overflow-hidden">
      ...
      <div className="flex items-center gap-2">
        <button className="bg-yellow-400 hover:bg-yellow-500 text-red-700 font-bold px-2 py-1 rounded-lg shadow-md transition-colors">
          {translate("download")}
        </button>
        <button
          className="text-white hover:text-red-400 text-sm transition-colors"
          onClick={() => setShowBanner(true)} // Fixed: now hides banner
        >
          <FaTimes />
        </button>
      </div>
