  ...
  const handleApply = async (e) => {
    ...
    if (String(selectedProcessTab).toLowerCase() === "opay") {
      if (opayOnlineCount > 0) {
        ...
        const userIdentifyAddress =
          user?.name || user?.username || userId || "guest";
    ...
    <div className="w-full">
            ...
            <div className="flex gap-4 flex-wrap mb-4">
              {amounts.map((amount) => (
                <div
                  ...
                  className={`relative px-6 py-3 rounded-lg border-2 cursor-pointer transition-all text-lg font-medium ${
      ...
      <div className="mt-10 text-center">
        <button
          onClick={handleApply}
          disabled={isLoading}
          className="px-12 py-5 rounded-xl text-black border-4 border-black font-bold text-xl shadow-2xl hover:shadow-3xl transition-all transform hover:scale-105 flex items-center justify-center gap-3 mx-auto disabled:opacity-70 disabled:cursor-not-allowed"
          style={{
